...
- **FR-043**: System MUST provide consistent theming across ALL UI elements (main window, dialogs, panels, menus) with three supported themes: Light (default - white background, black text), Dark (black background, white text), and System (follows OS theme). Theme selection must be accessible via Edit → Preferences with immediate application (no restart required). All controls including buttons, text fields, labels, and dialogs must inherit theme colors with proper contrast ratios meeting WCAG 2.1 AA standards (minimum 4.5:1 for normal text, 3:1 for large text).
- **FR-044**: System MUST provide editor-specific styling with open-source fixed-width font (default: JetBrains Mono 11pt or Consolas if unavailable), respecting active theme colors, and toggleable features including line numbers, current line highlighting, and syntax highlighting for Markdown.
